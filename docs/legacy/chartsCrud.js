...
import { isHttpUrl, resolveStoredAssetUrl } from '../../utils/assetUrls.js';
import { archiveDiscordAttachment, getChartArchiveChannelId } from '../../utils/discordMediaArchive.js';
import {
...
async function uploadChartPage({ typeKey, variantKey, attachment }) {
  return archiveDiscordAttachment({
    attachment,
    channelId: getChartArchiveChannelId(),
    label: `Chart: ${normalizeTypeKey(typeKey)} / ${normalizeChartVariantKey(variantKey)}`,
  });
}
...
export async function handleChartsCrudInteraction(interaction) {
  ...
  if (interaction.isButton() && id.startsWith('chartedit_')) {
    ...
    if (action === 'delvariant') {
      ...
      const removedPaths = (removed?.pages || []).map((p) => p?.asset_path).filter(Boolean).filter((p) => !isHttpUrl(p)).map(String);
    ...
    if (action === 'img') {
      ...
      try {
        ...
        if (currentPage?.asset_path && !isHttpUrl(currentPage.asset_path)) {
          const supabase = getSupabase();
    ...
    if (action === 'remove') {
      ...
      if (removed?.asset_path && !isHttpUrl(removed.asset_path)) {
        const supabase = getSupabase();
