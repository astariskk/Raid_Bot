...
import { archiveDiscordAttachment, getGifArchiveChannelId } from '../../utils/discordMediaArchive.js';
...
async function uploadAssetFromAttachment({ command, attachment, kind }) {
  ...
  const archivedUrl = await archiveDiscordAttachment({
    attachment,
    channelId: getGifArchiveChannelId(),
    label: `GIF/text command: ${safeCmd || command}`,
  });

  await updateGifCommandImage(safeCmd, archivedUrl);
  return archivedUrl;
}
...
function buildPreviewEmbed({ command, kind, row }) {
  ...
  if (kind === 'text') {
    ...
    const url = resolveStoredAssetUrl({ supabase, bucket, value: maybePath }) || '';
  ...
  if (row?.asset_path || row?.image_path) {
    const bucket = process.env.SUPABASE_GIF_BUCKET || 'gif-commands';
    const supabase = getSupabase();
    const url = resolveStoredAssetUrl({ supabase, bucket, value: row.asset_path || row.image_path });
    if (url) embed.setImage(url);
  } else {
