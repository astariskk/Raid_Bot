    ...
    client.on('messageCreate', async (message) => {
        ...
        if (message.content.toLowerCase() === '!pretendnewmonth') {
            ...
            try {
                ...
                if (raidChannel && raidChannel.isTextBased()) {
                    ...
                    if (guild) {
                        const embed = await createPaginatedLeaderboardEmbed({ // Use the new function for consistency
                            currentPage: 1,
                            totalPages: 1, // Only showing one page for the "final" leaderboard
                            usersData: topPlayersBeforeReset,
                            resetInfo: "Final Leaderboard for Last Month (Simulated Reset)",
                            originalRequesterId: message.author.id, // Dummy ID for this
                    ...
                    const newTopPlayers = getSortedLeaderboard(newLeaderboard, 10, true); // Get top 10 for new leaderboard
                    const newResetInfo = `Last reset: ${new Date().toLocaleDateString()}`;
                    const newEmbed = await createPaginatedLeaderboardEmbed({ // Use the new function for consistency
                        currentPage: 1,
                        totalPages: 1,
                        usersData: newTopPlayers,
                        resetInfo: newResetInfo,
                        originalRequesterId: message.author.id,
                        timestamp: Date.now()
                    }, client, message.guild);
                    await raidChannel.send({ content: "Here's the new leaderboard after the simulated reset:", embeds: newEmbed.embeds, components: [] });

                    // Trigger a backup after the simulated reset.
                    await sendLeaderboardBackup(client);
                ...
                await message.reply({ content: 'Failed to simulate monthly reset. Please check bot permissions and channel ID.', ephemeral: true });
            }
        }
    });
