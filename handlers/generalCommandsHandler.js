...
const realSecretTextGifCommands = {
    'marfart': '. [**MISS MARA**](https://files.catbox.moe/xwpnq5.gif)',
    'ahranus': '. [**THIS IS NOT PAINLESS LIKE KEVIN SAID AHHHHHH**](https://files.catbox.moe/och1y0.gif)',
    'ranfart': '. [**KEVIN GOT ME SHARTING FLAMES GYATTT**](https://files.catbox.moe/s3ql75.gif)'
};
...
export function setupGeneralCommandsHandler(client) {
    client.on('messageCreate', async (message) => {
        ...
        // Helper function to handle GIF sending with cooldown and optional self-destruction
        const handleGifCommand = async (commandMap, commandKey, isSecret = false) => {
            if (lastUsed && (now - lastUsed < GIF_COOLDOWN_DURATION)) {
                ...
                return true; // Indicate that cooldown was hit
            }
            ...
            let sentMessage;
            try {
                const gifInfo = commandMap[commandKey];
                if (typeof gifInfo === 'string') { // Text GIF command
                    sentMessage = await message.channel.send(gifInfo);
                } else { // Embed GIF command
                    const gifEmbed = new EmbedBuilder()
                        .setColor(gifInfo.color)
                        .setTitle(gifInfo.title)
                        .setImage(gifInfo.image)
                        .setFooter({ text: gifInfo.footer });
                    sentMessage = await message.channel.send({ embeds: [gifEmbed] });
                }

                if (isSecret && sentMessage) {
                    setTimeout(async () => {
                        try {
                            // Attempt to edit the message
                            await sentMessage.edit('hello');
                            // Then delete it
                            await sentMessage.delete();
                        } catch (err) {
                            if (err.code === 10008) { // Unknown Message
                                console.warn(`Attempted to delete already deleted secret GIF message for user ${userId}.`);
                            } else {
                                console.error(`Error editing or deleting secret GIF message for user ${userId}:`, err);
                            }
                        }
                    }, 5000); // 5 seconds
                }
            } catch (error) {
                console.error(`Error sending GIF for command "${commandKey}":`, error);
                await message.channel.send('Could not display the requested GIF.');
            }
            return false; // Indicate that command was executed
        };
...
`
        ...
        // --- Handle !secretcommands to list all *publicly known* GIF commands ---
        ...
        // Check for real secret gif commands (with embeds) FIRST, as they have special behavior
        if (realSecretGifCommands[commandContent]) {
            await handleGifCommand(realSecretGifCommands, commandContent, true);
        }
        // Check for real secret text gif commands (no embeds)
        else if (realSecretTextGifCommands[commandContent]) {
            await handleGifCommand(realSecretTextGifCommands, commandContent, true);
        }
        // Check for regular gif commands (with embeds)
        else if (gifCommands[commandContent]) {
            await handleGifCommand(gifCommands, commandContent);
        }
        // Check for text gif commands (no embeds)
        else if (textGifCommands[commandContent]) {
            await handleGifCommand(textGifCommands, commandContent);
        }
    });
